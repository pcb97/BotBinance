...
class Strategy:
    ...
    def parse_trades(self, candle:Candle) -> str:
        ...
        # New Candle
            ...
            self.client._add_log(f"{self.exchange} NOVO CANDLE PARA {self.contract.symbol} {self.tf}",tipo='ALERTA')
            return "new_candle"
            ...
            if order_status.status == "filled":
                for trade in self.trades:
                    if trade.entry_id == order_id:
                        ...
                        threading.Thread(target=self.actualize_stop_loss, args=(trade,self.client)).start() #ATUALIZA O STOPLOSS A PARTIR DO AVR
    ...
    def check_stop_loss_status(self,binance):
        while self in binance.strategies.values():
            try:
                if self.last_sl!=None:
                    status_stop = self.last_sl
    ...
    def actualize_stop_loss(self,trade,binance):
        while self in binance.strategies.values():
            ...
            high = pd.Series([x.high for x in self.candles])
            low = pd.Series([x.low for x in self.candles])
            ...
            true_range['true_range'] = true_range.apply(lambda x:max(x), axis=1)
            atr_series = true_range['true_range'].rolling(window=atr_period).mean()
            
            if trade.side.upper() == "BUY":
                if self.candles[-1].close > self.candles[-2].close:
                    self.stop_mov = self.candles[-1].close - (atr_multiplier * atr_series.iloc[-1])
            else:
                if self.candles[-1].close < self.candles[-2].close:
                    self.stop_mov = self.candles[-1].close + (atr_multiplier * atr_series.iloc[-1])
            if trade.status == "open":
                if (trade.side.upper() == "BUY" and self.stop_mov > self.stop_price) or (trade.side.upper() != "BUY" and self.stop_mov < self.stop_price):
                    self.stop_price = self.stop_mov
                    order_sm = self.client.place_orderV2(self.contract, "STOP_MARKET",trade.quantity,trade.side.upper(), self.stop_price, tipo='ATUALIZACAO STOPLOSS')
                    if order_sm!=None:
                        self.client.cancel_order(self.contract, self.last_sl.order_id)
                        self.last_sl = order_sm
    ...
    def _open_position(self, signal_result: int):
        ...
        if self.ongoing_position: #CANCELANDO POSICAO ANTERIOR, SE JA ESTIVER EM UMA
            for trade in [x for x in self.trades if x.status=='open' and x.entry_price!=None]:
                side_fec = 'BUY' if trade.side.upper()=='SELL' else "SELL"
                order_status = self.client.place_orderV2(self.contract, "MARKET", trade.quantity, side_fec, tipo="FINALIZAR POSICAO")
            ...
            order_cancel = self.client.cancel_order(self.contract, self.last_sl.order_id)
        ...
        if order_status is not None:
            ...
            if order_side == "BUY":
                self.stop_price = (exec_price * (1 - self.stop_loss / 100))
            else:
                self.stop_price = (exec_price * (1 + self.stop_loss / 100))
            orderContraria = 'SELL' if order_side=='BUY' else 'BUY'

            #CRIANDO SL DA NOVA POSICAO
            order_sl = self.client.place_orderV2(self.contract, "STOP_MARKET", trade_size, orderContraria,self.stop_price, tipo='STOPLOSS NOVA POSICAO')        
            if order_sl!=None:
                self.last_sl = order_sl
            ...
            if avg_fill_price!=None:
                threading.Thread(target=self.actualize_stop_loss, args=(new_trade,self.client)).start()   #ATUALIZA O STOPLOSS A PARTIR DO AVR
                threading.Thread(target=self.enviarDadosTelegram, args=("NEW",)).start() #ENVIA DADOS PARA O TELEGRAM
            self.stop_mov = self.stop_price
    ...
    def _check_tp_sl(self, trade: Trade):
        ...
        if tp_triggered:
            ...
            sideCancel = "BUY" if trade.side.upper()=='SELL' else "SELL"
            order_status = self.client.place_orderV2(self.contract, "MARKET", trade.quantity, sideCancel, tipo="SL/TP")            
            if order_status is not None:
                cancelOrdem = self.client.cancel_order(self.contract, self.last_sl.order_id)
                self.client._add_log(f"Exit order on {self.contract.symbol} {self.tf} placed successfully")
                trade.status = "closed"
                self.ongoing_position = False
                threading.Thread(target=self.enviarDadosTelegram, args=("CLOSE",)).start() #ENVIA DADOS PARA O TELEGRAM
...
class MovingaverageStrategy(Strategy):
    ...
    def check_trade(self, tick_type: str):
        ...
        :return:
        """
        if not self.EstrategiaPronta:
            self.client._add_log(f"ATUALIZANDO DADOS ENTRADA PARA {self.contract.symbol}",tipo="INFO")
            signal_result = self._check_signal()
            self.EstrategiaPronta = True
