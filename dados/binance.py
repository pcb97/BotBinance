...
class BinanceClient:
                ...
                for posicaoAberta in posicoesAberta:
                    ...
                    if len(dadosPosicoes)>0:
                        ...
                        if new_trade.side.upper() == "BUY":
                            strategy.stop_price.append(new_trade.entry_price * (1 - strategy.stop_loss / 100))
                        else:
                            strategy.stop_price.append(new_trade.entry_price * (1 + strategy.stop_loss / 100))
                        strategy.trades.append(new_trade)
                        ORDEM = OrderStatus(posicao, self.platform)
                        strategy.last_sl.append(ORDEM)
                        strategy.stop_mov.append(strategy.stop_price[-1])
                        threading.Thread(target=strategy.actualize_stop_loss, args=(new_trade,)).start() #ATUALIZA O STOPLOSS A PARTIR DO AVR
    ...
    def lerDadosEntrada(self):
        try:
            ...
            s3 = boto3.client('s3')
            dados = pd.read_excel(BytesIO(s3.get_object(Bucket = 'binance-trade', Key = 'DadosEntrada.xlsx')['Body'].read()),sheet_name='DADOS').to_dict(orient='records')
            print("DADOS DO S3")
        except:
    ...
    def rodar(self):
        ...
        for strategy in self.strategies.values():
            threading.Thread(target=strategy.check_stop_loss_status).start() #FICA SEMPRE VERIFICANDO O STOPLOSS
