...
from utilsTelegram import BotTelegram
class BinanceClient:
    def __init__(self, public_key: str, secret_key: str, testnet: bool, futures: bool, idTelegram=""):
        ...
        self.telegram = BotTelegram(ID=idTelegram)
        ...
        threading.Thread(target=self.rodar).start()
    ...
    def place_orderV2(self, contract: Contract, order_type: str, quantity: float, side: str, price=None,
        ...
        data['signature'] = self._generate_signature(data)
        self._add_log("="*100)
        self._add_log(f"CRIANDO ORDEM {operacao} PARA {data['symbol']}")
        self._add_log(f"LADO: {data['side']}")
        self._add_log(f"QUANTIDADE: {data['quantity']}")
        if price:
            self._add_log(f"PREÇO: {price}")
        self._add_log(f"TIPO: {tipo}")
        self._add_log(f"HORA: {datetime.datetime.today():%H:%M:%S %d/%m/%Y}")
    ...
    def pegarPosicoesAbertas(self):
        ...
        for strategia in self.strategies.values(): #PEGANDO POSICOES ABERTAS
            strategia.trades = []
            strategia.last_sl = []
            strategia.stop_price = []
            strategia.stop_mov = []
            data={}
            data['timestamp'] = self.timestamp()
            data['symbol'] = strategia.contract.symbol
            data['signature'] = self._generate_signature(data)
            ...
            if len(posicoesAberta)>0: # SE TIVER ATIVO COMPRADO PARA AQUELA STRATEGIA
                for posicaoAberta in posicoesAberta:
                    ...
                    if len(dadosPosicoes)>1:
                        ...
                        for i,posicao in enumerate(dadosPosicoes[:-1]): #CANCELA TODAS AS ORDENS, MENOS A ULTIMA
                            ...
                            self.cancel_order(strategia.contract,posicao['orderId'])
                    if len(dadosPosicoes)>0:
                        ...
                        if new_trade.side.upper() == "BUY":
                            strategia.stop_price.append(new_trade.entry_price * (1 - strategia.stop_loss / 100))
                        else:
                            strategia.stop_price.append(new_trade.entry_price * (1 + strategia.stop_loss / 100))
                        strategia.trades.append(new_trade)
                        ORDEM = OrderStatus(posicao, self.platform)
                        strategia.last_sl.append(ORDEM)
                        strategia.stop_mov.append(strategia.stop_price[-1])
                        threading.Thread(target=strategia.actualize_stop_loss, args=(new_trade,)).start() #ATUALIZA O STOPLOSS A PARTIR DO AVR
                        strategia.ongoing_position = True
                    else:
                        self._add_log(f"NENHUMA ORDEM SL EM ABERTO. *CHECAR ATIVO NA BINANCE*")
                        strategia.ongoing_position = False
                    self._add_log("="*100)
    ...
    def lerDadosEntrada(self):
        ...
        for i,info in enumerate(dados): #CRIA AS ESTRATEGIAS A PARTIR DO EXCEL
            ...
            new_strategy.candles = self.get_historical_candles(contrato, info['TimeFrame'])[:-2]
    ...
    def rodar(self):
        ...
        if self.telegramOK: threading.Thread(target=self.telegram.GetMessageUser,args=(self,)).start()
        self.executar = True
        for strategia in self.strategies.values():
            threading.Thread(target=strategia.check_stop_loss_status).start() #FICA SEMPRE VERIFICANDO O STOPLOSS
        ...
        while True:
            try:
                ...
                import traceback
                self._add_log(traceback.format_exc(),tipo="CRITICO")
                # return self.rodar()
                return
