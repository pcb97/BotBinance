...
updateLink = 'https://drive.google.com/uc?export=download&id=17AUB3xInq1wOMrhLWf5LqkTNEuuHl1qo'
...
class BotTelegram:
	...
	def GetMessageUser(self,binance,i=0):
		try:
			...
			def getMessage(message):
				...
				if "grafico" in mensagem:
				...
				elif 'lerposicoes' in mensagem:
					binance.pegarPosicoesAbertas()
					self.sendMessage("Posições abertas lidas",message.message_id)
					...
					if binance.EC2:
						...
						dados.extractall()
					self.sendMessage("ROBO ATUALIZADO COM SUCESSO. REINICIANDO SERVIDOR",message.message_id)
					import sys
	...
	def getID(self):
		...
		print("ABRA UMA CONVERSA COM O BOT CRIADO")
